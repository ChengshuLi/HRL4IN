...
from gibson2.envs.motion_planner_env import MotionPlanningBaseArmHRL4INEnv
...
def evaluate(args,
    ...
    episode_spls = torch.zeros(envs._num_envs, 1, device=device)
    ...
    while episode_counts.sum() < args.num_eval_episodes:
        with torch.no_grad():
            ...
            batch["subgoal"] = current_subgoals
        ...
        spls = torch.tensor(
            [[float(info["spl"])] if done and "spl" in info else [0.0]
             for done, info in zip(dones, infos)],
            dtype=torch.float,
            device=device
        )
...
def main():
    ...
    # # base, arm, base + arm
    # subgoal_mask_choices = torch.zeros(3, arm_world_space.shape[0], device=device)
    # subgoal_mask_choices[0, 0:2] = 1.0
    # subgoal_mask_choices[1, :] = 1.0
    ...
    for update in range(start_epoch, args.num_updates):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                ...
                rollouts.observations["subgoal"][step] = current_subgoals
            ...
            # print('-' * 50)
            # print("prev_obs", step_observation["arm_world"])
            # print("current_subgoals", current_subgoals)
            # print("current yaw", step_observation["yaw"])
            # print("ideal_next_state", ideal_next_state)
            # print("next_obs", next_obs_batch["arm_world"])
        ...
        window_episode_spls.append(episode_spls.clone())
        ...
        with torch.no_grad():
            ...
            last_observation["subgoal"] = current_subgoals
